...
struct Color
{
    ...
    inline Color(Transparency _v) : c{Fl_Color()}, v{static_cast<unsigned char>(_v)} {}
...
struct Line_style
{
    ...
    inline Line_style(Line_style_type _points, pix_amount _width) : s{_points}, width{_width} {}
...
class Shape
{
  public:
    ...
    inline Shape& operator= (const Shape&) = delete;  // (предотвращение копирования)
...
class Marked_polyline : public Open_polyline
{
  public:
    ...
    inline Marked_polyline(const std::string& _mark, std::initializer_list<Point> _points)
        : Open_polyline{_points}, mark{_mark}
    {
    }

    inline Marked_polyline(std::initializer_list<Point> _points) : Open_polyline{_points} {}
...
{
  public:
    inline Marks(const std::string& m) : Marked_polyline{m} { set_color(Color{Color::invisible}); }
};
...
class Bad_image : public Fl_Image
{
  public:
    ...
    inline void draw (int x, int y, int, int, int, int) override { draw_empty(x, y); }
...
class Image : public Shape
{
  ...
  private:
    ...
    pix_amount cx, cy;  // define "masking box" within image relative to position (cx,cy)
    Fl_Image* img_ptr;
    Text text;
