...
void Button::attach(Window& win)
{
    ...
    ptr_wid = new Fl_Button{int(loc.x), int(loc.y), int(width), int(height), label.c_str()};
    // (великий и ужасный...)
...
{
    // (FLTK работает на int'ах, так что преобразуем)
    ptr_wid = new Fl_Input{int(loc.x), int(loc.y), int(width), int(height), label.c_str()};
    own = &win;
}

//-------------------------------------------------------------------------------------------------------------------------------

void Out_box::put(const std::string& s) { reference_to<Fl_Output>(ptr_wid).value(s.c_str()); }

void Out_box::attach(Window& win)
{
    // (FLTK работает на int'ах, так что преобразуем)
    ptr_wid = new Fl_Output{int(loc.x), int(loc.y), int(width), int(height), label.c_str()};
    own = &win;
}
