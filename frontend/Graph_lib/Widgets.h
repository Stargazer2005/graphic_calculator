...
using Callback = void (*)(void*, void*);  // Обязательный тип функции FLTK для всех обратных вызовов

template <class W> W& reference_to (void* ptr_wid) { return *static_cast<W*>(ptr_wid); }
...
class Widget
{
  public:
    ...
    inline Widget(Point _loc, pix_amount _width, pix_amount _height, const std::string& _label,
                  Callback _cb)
...
{
    inline Button(Point _loc, pix_amount _width, pix_amount _height, const std::string& _label,
                  Callback _cb)
...
{
    inline In_box(Point _loc, pix_amount _width, pix_amount _height, const std::string& s)
...
{
    inline Out_box(Point _loc, pix_amount _width, pix_amount _height, const std::string& s)
        : Widget{_loc, _width, _height, s, nullptr}
    {
    }
