...
Function_box::Function_box(unsigned long long int graphics_amount, void cb_draw_graph(void*, void*),
    ...
    : Graph_lib::Widget{Point{0, func_box_h * int(graphics_amount)}, func_box_w, func_box_h, "",
      ...
      hide_button{
          new Numbed_button{Point{btn_w + padding, func_box_h * int(graphics_amount) + in_box_h},
                            btn_w, btn_h, "Hide", cb_hide_graph}},
      rem_button{new Numbed_button{
          Point{(btn_w + padding) * 2, func_box_h * int(graphics_amount) + in_box_h}, btn_w, btn_h,
          "Remove", cb_rem_func}},
      curr_der{new Out_box{Point{0, func_box_h * int(graphics_amount) + in_box_h + btn_h}, btn_w,
                           btn_h, ""}},
      draw_der_button{new Numbed_button{
          Point{btn_w + padding, func_box_h * int(graphics_amount) + in_box_h + btn_h}, btn_w,
          btn_h, "Draw", cb_draw_der}},
      hide_der_button{new Numbed_button{
          Point{(btn_w + padding) * 2, func_box_h * int(graphics_amount) + in_box_h + btn_h}, btn_w,
          btn_h, "Hide", cb_hide_der}},
      out_box{new Out_box{Point{0, func_box_h * int(graphics_amount) + in_box_h + 2 * btn_h},
                          out_box_w, out_box_h, ""}}
{
}
