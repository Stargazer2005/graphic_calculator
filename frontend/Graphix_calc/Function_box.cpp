...
Function_box::Function_box(unsigned long long int box_index, void cb_graphix_draw(void*, void*),
                           void cb_graphix_hide(void*, void*), void cb_rem_func(void*, void*),
                           void cb_deriv_draw(void*, void*), void cb_deriv_hide(void*, void*))
    : draw_graphix_button{new Numbed_button{Point{0, func_box_h * pix_amount(box_index) + in_box_h},
                                            btn_w, btn_h, "Draw", box_index, cb_graphix_draw}},
      hide_graphix_button{new Numbed_button{
          Point{btn_w + margin / 2, func_box_h * pix_amount(box_index) + in_box_h}, btn_w, btn_h,
          "Hide", box_index, cb_graphix_hide}},
      draw_deriv_button{new Numbed_button{
          Point{btn_w + margin / 2, func_box_h * pix_amount(box_index) + in_box_h + btn_h}, btn_w,
          btn_h, "Draw", box_index, cb_deriv_draw}},
      hide_deriv_button{new Numbed_button{
          Point{(btn_w + margin / 2) * 2, func_box_h * pix_amount(box_index) + in_box_h + btn_h},
          btn_w, btn_h, "Hide", box_index, cb_deriv_hide}},
      out_der_box{new Out_box{Point{0, func_box_h * pix_amount(box_index) + in_box_h + btn_h},
                              btn_w, btn_h, empty_str}},
      // in_box_label_w: необходимо отступить расстояние, где находится "y = "
      in_box{new In_box{Point{in_box_label_w, func_box_h * pix_amount(box_index)}, in_box_w,
                        in_box_h, "y = "}},
      out_box{new Out_box{Point{0, func_box_h * pix_amount(box_index) + in_box_h + 2 * btn_h},
                          out_box_w, out_box_h, empty_str}},
      rem_func_button{new Numbed_button{
          Point{(btn_w + margin / 2) * 2, func_box_h * pix_amount(box_index) + in_box_h}, btn_w,
          btn_h, "Remove", box_index, cb_rem_func}}
{
}
