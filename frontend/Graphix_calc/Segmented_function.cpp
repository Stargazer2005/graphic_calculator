...
Segmented_function::Segmented_function(string str, int scale, Point center, int max_x, int max_y)
    : expr{function_string(str)}, segs{segments(max_x, max_y, scale)},
...
vector<Segment> Segmented_function::segments(int max_x, int max_y, int scale) const
{
    ...
    vector<Segment> res = domain_segments(expr, l_border, r_border, h_border, prec_seg);
    return res;
}
                                                            ...
                                                            int max_x) const
{
    ...
    for (auto seg : segs)
    {
        Function* f = new Function{expr.calculate, seg.start,     seg.end,      center,
