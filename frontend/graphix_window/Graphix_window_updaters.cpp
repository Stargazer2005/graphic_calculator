    ...
    if (enter_menu[func_index]->is_input_valid())
    {
        ...
        // иначе мы просто рисуем простой график
            ...
            seged_graphix = new Segmented_graphix{new Graphix{
                func.calculate, converted_to_real({border_dist, h()}, origin, unit_intr).x,
                converted_to_real({w(), 0}, origin, unit_intr).x, origin, 2, unit_intr}};
...
void Graphix_window::update_points()
{
    ...
    Math_calc::Point left_bottom = converted_to_real({func_box_w, h()}, origin, unit_intr);
