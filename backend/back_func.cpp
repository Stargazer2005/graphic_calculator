...
{
  const string calc_chars =
      "x1234567890+-*/^sctelu()";  // строка со всеми разрешенными символами
  const string oper = "+-*/^";  // строка с операциями

  // проверка на скобочки
  if (count(expr.begin(), expr.end(), '(') !=
      count(expr.begin(), expr.end(), ')'))
  {
  ...
  {
    // throw invalid_argument("checker: expression begin or end with
    // unclosed operation sign");
    return false;
  }
  for (unsigned long long i = 0; i < expr.size(); i++)
  {
    ...
    if (c_in_s(c, oper) && c != '-')
    {
      if ((c_in_s(expr[i - 1], oper) || c_in_s(expr[i + 1], oper)) &&
          (expr[i - 1] != '-' && expr[i + 1] != '-'))
      {
        // throw invalid_argument("checker: multiple use of operation sign
        // in one place");
        return false;
      }
    }
...
double calc (const vector<string>& rev_pol, double x)
{
  ...
  for (auto lex : rev_pol)
  {
    ...
    char curr =
        transform_to_char(lex);  // текущий символ,
                                 // если брать вместо строки (для switch)
    if (c_in_s(curr, func))
    {
      l = stack.last();  // запоминаем только последний, так как операции
                         // унарны
      stack.pop();
      switch (curr)
      {
