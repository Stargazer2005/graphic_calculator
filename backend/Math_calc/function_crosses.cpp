...
function_crosses::function_crosses(
    ...
    // рассматриваем иксы пересечения мат. функций,
    // как точки-корни для мат. функции - их разницы
...
vector<Point> function_crosses::crosses(Point left_bottom,
  ...
  for (const auto& seg :
    ...
    for (const auto& local_seg : estimated_segment(seg)) {
      ...
      double y = f(x);
      // отличие от нахождения корней заключается в том,
      // что теперь нам не нужна проверка на приближенность нулю
