  ...
  for (size_t i = 0; i < func_str.size(); i++) {
    ...
    if (ch == 'y') {
      ...
      if (est_number > all_funcs_str.size())
        throw invalid_argument("invalid function number: " + read_est_number);

      // встретили внутри еще не до конца раскрытых зависимостей повтор номера
      if (elem_in_vec(est_number, unexposed_dependencies))
      ...
      if (c_in_s('y', all_funcs_str[est_index]))
        unexposed_dependencies.push_back(est_number);
      // если не содержит, то раскрыли веточку
      else
        unexposed_dependencies.clear();

      // если неполных раскрытий с этим номером не было,
      // добавляем его в общий список
      // (таким образом в общем списке не будет повторов)
      if (!elem_in_vec(est_number, dependencies))
        dependencies.push_back(est_number);

      // заменяем в func_str выражение y_n на то, что оно означает
      // (заключаем в скобки, чтобы не нарушать мат. порядок действий)

      // TODO: выводить полный тип ошибки только в боксе первичной мат. функции
      ...
      if (!all_funcs_str[est_index].empty())
        Backend_utilities::replace(func_str, "y_" + read_est_number,
